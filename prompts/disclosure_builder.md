...
`embodiments` 至少包含两个实施方式；除非用户明确要求简版，优先写 3–5 个实施方式或优选实现，使代理人能从不同角度理解可实施范围。
...
实施方式三：……优选实现/配置方式/异常处理方式/离线处理方式
……
```

若用户参考样稿以“步骤一、步骤二”呈现，可沿用该风格，但仍须至少覆盖两个实施方式，并优先补充多个优选实现。软件、算法、平台类方案可按以下角度扩展：实时在线实施方式、离线批处理实施方式、系统/模块实施方式、参数配置实施方式、异常/容错实施方式、替代模型或部署实施方式。
...
`drawings` 至少给出图1；除非方案很简单，至少生成 2 张附图，优先生成 3–5 张。建议图1为系统整体架构图，图2为核心方法流程图，后续可根据技术内容补充模块交互图、数据流/状态流图、关键判断逻辑图、部署结构图或实施例结构图。
...
选择附图数量时，优先覆盖不同技术层面，而不是把同一流程拆成重复图：

- 系统架构图：展示模块组成、上下游关系；
- 方法流程图：展示 S1-Sn 的处理顺序和分支；
- 模块交互图：展示请求、响应、回调、状态写入或存储关系；
- 数据流/状态流图：展示输入数据、特征、判断结果、状态码或输出结果；
- 关键判断逻辑图：展示阈值、规则、模型结论、异常处理或 fail-open/fail-close 等逻辑。
