...
4. **实施方式尽量充分**：除非用户明确要求简版，所有的实施方式栏至少包含“实施方式一”和“实施方式二”，并优先扩展为 3–5 个实施方式或优选实现。方法类可按 S1-Sn 展开；系统/设备类需结合模块和附图标记说明；软件/算法类应尽量覆盖实时处理、离线处理、配置/参数、异常处理、替代模型或部署形态等不同实施角度。
5. **附图说明规范**：附图统一用“图1、图2、图3……”；图说明用“图1为……图”的句式。除非方案很简单，附图至少 2 张，优先生成 3–5 张，包括系统架构图、方法流程图、模块交互图、数据流/状态流图、关键判断逻辑图或实施例结构图。附图图片本体必须采用白底、黑色边框、黑色文字、黑色箭头的简洁流程卡片风格；根据实际关系选择纵向、横向或多层堆叠排布；图片内部不得写“图1”“图2”等图号、图题或描述性标题，图号和图题只写在 Word 正文图注/附图说明中。
